...
--- Ouvre un prompt pour le choix du projet a lancer
---@param items table avec les choix possibles
---@param prompt string Message a afficher
---@param label_fn function pour formatter les items
---@return string|nil le choix retourne par le picker
...
--- Ouvre un prompt pour le choix du projet a lancer
---@param items table choix a choisir
---@param prompt string message a afficher
---@param label_fn function pour formatter les items
---@param cb function callback lors du choix
...
function CMakeSetup()
  ...
  vim.notify("Project setup ", 1, nil)
end

function CMakeBuild()
  root_dir = vim.fs.dirname(vim.fs.find({ '.git' }, { upward = true })[1])
  os.execute('cd ' .. root_dir .. ' && ninja -C build')
end
...
dap.configurations.cpp = {
  {
    ...
    request = "launch",
    program = function()
      local name = pick_one(projects(), "Choose a project", function(e)
        return e
      end)
